...
class SemantickittiMsKdDataset(data.Dataset):
    def __init__(
        self,
        data_cfgs=None,
        training: bool = True,
        ...
        self.flexible_steps_gt = self.data_cfgs.FLEXIBLE_STEPS_GT
    ...
    def __getitem__(self, index):
        ...
        if int(self.annos[index][-10:-4]) - 1 >=0:
            raw_data_ms, annotated_data_ms,  annotated_data_ms_mask, annotated_data_ms_mask_gt \
                        = self.multiscan_fuse(self.annos, index, self.multiscan, self.flexible_steps, self.flexible_steps_gt)
            raw_data_ms_gt = np.concatenate([raw_data, raw_data_ms[annotated_data_ms_mask_gt]])
            raw_data_ms_gt = self.append_time_flag(raw_data, raw_data_ms_gt)
        ...
        if self.augment == 'GlobalAugment_LP':
            if self.split == 'train' and prob == 1:
                ...
                if int(self.annos_another[index][-10:-4]) - 1 >=0:
                    raw_data1_ms, annotated_data1_ms,  annotated_data1_ms_mask, annotated_data1_ms_mask_gt \
                                = self.multiscan_fuse(self.annos_another, index, self.multiscan, self.flexible_steps, self.flexible_steps_gt)
                    raw_data1_ms_gt = np.concatenate([raw_data, raw_data1_ms[annotated_data1_ms_mask_gt]])
                    raw_data1_ms_gt = self.append_time_flag(raw_data, raw_data1_ms_gt)
                ...
                annotated_data_ms_gt = np.zeros((len(raw_data_ms_gt), 1), dtype=raw_data_ms_gt.dtype)
                annotated_data1_ms_gt = np.zeros((len(raw_data1_ms_gt), 1), dtype=raw_data1_ms_gt.dtype)
                raw_data_ms_gt, annotated_data_ms_gt, strategy_ms_gt = lasermix_aug(raw_data_ms_gt, 
                        annotated_data_ms_gt, raw_data1_ms_gt, annotated_data1_ms_gt, strategy=strategy, return_strategy=True)
                assert (strategy==strategy_ms) and (strategy==strategy_ms_gt)
                ...
                annotated_data_ms_gt = np.zeros((len(raw_data_ms_gt)), dtype=raw_data_ms_gt.dtype)
                annotated_data1_ms_gt = np.zeros((len(raw_data1_ms_gt)), dtype=raw_data1_ms_gt.dtype)
                raw_data_ms_gt, annotated_data_ms_gt, swap_flag_ms_gt, rotate_flag_ms_gt = polarmix(
                    raw_data_ms_gt, annotated_data_ms_gt, raw_data1_ms_gt, annotated_data1_ms_gt,
                    alpha=alpha, beta=beta, instance_classes=instance_classes, 
                    Omega=Omega, swap_flag=swap_flag, rotate_flag=rotate_flag, return_strategy=True
                )

                assert (swap_flag==swap_flag_ms) and (swap_flag==swap_flag_ms_gt)
                assert (rotate_flag==rotate_flag_ms) and (rotate_flag==rotate_flag_ms_gt)
        ...
        ringID_ms_gt = self.get_kitti_points_ringID(raw_data_ms_gt).reshape((-1,1))
        raw_data_ms_gt= np.concatenate([raw_data_ms_gt, ringID_ms_gt.reshape(-1, 1)], axis=1).astype(np.float32)
        pc_data = {
            'xyzret': raw_data,
            'xyzret_ms': raw_data_ms,
            'xyzret_ms_gt': raw_data_ms_gt,
    ...
    def multiscan_fuse(self, annos, index, multiscan, flexible_steps, flexible_steps_gt):
        raw_data_ms = []
        annotated_data_ms = []
        annotated_data_ms_mask = []
        annotated_data_ms_mask_gt = []
        ...
        for delta_idx in range(-multiscan, multiscan):
            ...
            if (self.split == 'test') and (self.seq == -1):
                ...
                if newpath.split('/sequences/')[-1][:2] in self.trainval_seqs:
                    ...
                    annotated_data_pseudo_gt = annotated_data
            ...
            annotated_data_mask_gt = np.zeros(len(annotated_data), dtype=np.bool)
            for class_idx, class_step_gt in enumerate(flexible_steps_gt):
                if class_step_gt == 0:
                    continue
                if np.abs(delta_idx) % class_step_gt == 0:
                    annotated_data_mask_gt = annotated_data_mask_gt | (annotated_data_pseudo_gt[:,0] == LEARNING_MAP_INV[class_idx]) # 此处代码有问题
            ...
            annotated_data_ms_mask_gt.append(annotated_data_mask_gt)
        ...
        annotated_data_ms_mask_gt = np.concatenate(annotated_data_ms_mask_gt, 0)

        annotated_data_ms = np.vectorize(LEARNING_MAP.__getitem__)(annotated_data_ms)
        return raw_data_ms, annotated_data_ms, annotated_data_ms_mask, annotated_data_ms_mask_gt
