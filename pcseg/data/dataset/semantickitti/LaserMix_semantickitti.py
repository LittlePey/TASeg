...
def lasermix_aug(xyzi_sup, label_sup, xyzi_unsup, label_unsup, strategy='mixture', return_strategy=False):
    ...
    if return_strategy:
        return Xyzi, Label, strategy
    else:
        return Xyzi, Label

def lasermix_aug_(xyzi_sup, label_sup, xyzi_unsup, label_unsup, strategy='mixture', return_strategy=False):
    'Generates one sample of data'   # for semantic-kitti
    # Mix sup and unsup for MixTeacher training
    global xyzil_mix_1
    xyzil_sup = np.concatenate((xyzi_sup,  label_sup), axis=1)  # [N, 5]
    ...
    inc_sup = np.arctan2(xyzil_sup[:, 2], rho_sup) / np.pi * 180  # inc = arctan(z / (x^2 + y^2)^(1/2)) / np.pi * 180 ∈(-20, 5)
    inc_unsup = np.arctan2(xyzil_unsup[:, 2], rho_unsup) / np.pi * 180  # inc = arctan(z / (x^2 + y^2)^(1/2)) / np.pi * 180 ∈(-20, 5)

    if strategy == 'mixture':
        # strategies = ['rho3phi1', 'rho3phi2', 'rho4phi1', 'rho4phi2', 'rho5phi1', 'rho5phi2', 'rho6phi1', 'rho6phi2',]
        # strategies = ['inc3phi1', 'inc3phi2', 'inc4phi1', 'inc4phi2', 'inc5phi1', 'inc5phi2', 'inc6phi1', 'inc6phi2']
        strategies = ['inc3phi1', 'inc4phi1', 'inc5phi1', 'inc6phi1'] # 将[-20, 0] 均匀划分成多份进行mixture
        strategy = np.random.choice(strategies, size=1)[0]

    if strategy == 'inc3phi1':

        xyzil_sup_p1 = xyzil_sup[inc_sup > -6.7]
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= -6.7, inc_sup > -13.4)]
        xyzil_sup_p3 = xyzil_sup[inc_sup <= -13.4]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > -6.7]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= -6.7, inc_unsup > -13.4)]
        xyzil_unsup_p3 = xyzil_unsup[inc_unsup <= -13.4]
        ...
        xyzil_sup_p1 = xyzil_sup[inc_sup > -5.0]
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= -5.0, inc_sup > -10.0)]
        xyzil_sup_p3 = xyzil_sup[np.logical_and(inc_sup <= -10.0, inc_sup > -15.0)]
        xyzil_sup_p4 = xyzil_sup[inc_sup <= -15.0]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > -5.0]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= -5.0, inc_unsup > -10.0)]
        xyzil_unsup_p3 = xyzil_unsup[np.logical_and(inc_unsup <= -10.0, inc_unsup > -15.0)]
        xyzil_unsup_p4 = xyzil_unsup[inc_unsup <= -15.0]
        ...
        xyzil_sup_p1 = xyzil_sup[inc_sup > -4.0]
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= -4.0, inc_sup > -8.0)]
        xyzil_sup_p3 = xyzil_sup[np.logical_and(inc_sup <= -8.0, inc_sup > -12.0)]
        xyzil_sup_p4 = xyzil_sup[np.logical_and(inc_sup <= -12.0, inc_sup > -16.0)]
        xyzil_sup_p5 = xyzil_sup[inc_sup <= -16.0]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > -4.0]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= -4.0, inc_unsup > -8.0)]
        xyzil_unsup_p3 = xyzil_unsup[np.logical_and(inc_unsup <= -8.0, inc_unsup > -12.0)]
        xyzil_unsup_p4 = xyzil_unsup[np.logical_and(inc_unsup <= -12.0, inc_unsup > -16.0)]
        xyzil_unsup_p5 = xyzil_unsup[inc_unsup <= -16.0]
        ...
        xyzil_sup_p1 = xyzil_sup[inc_sup > -3.3]
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= -3.3, inc_sup > -6.6)]
        xyzil_sup_p3 = xyzil_sup[np.logical_and(inc_sup <= -6.6, inc_sup > -9.9)]
        xyzil_sup_p4 = xyzil_sup[np.logical_and(inc_sup <= -9.9, inc_sup > -13.2)]
        xyzil_sup_p5 = xyzil_sup[np.logical_and(inc_sup <= -13.2, inc_sup > -16.5)]
        xyzil_sup_p6 = xyzil_sup[inc_sup <= -16.5]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > -3.3]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= -3.3, inc_unsup > -6.6)]
        xyzil_unsup_p3 = xyzil_unsup[np.logical_and(inc_unsup <= -6.6, inc_unsup > -9.9)]
        xyzil_unsup_p4 = xyzil_unsup[np.logical_and(inc_unsup <= -9.9, inc_unsup > -13.2)]
        xyzil_unsup_p5 = xyzil_unsup[np.logical_and(inc_unsup <= -13.2, inc_unsup > -16.5)]
        xyzil_unsup_p6 = xyzil_unsup[inc_unsup <= -16.5]
