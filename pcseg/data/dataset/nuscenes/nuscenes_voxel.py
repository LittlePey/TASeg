...
from .nuscenes_sweep import NuscenesSweepDataset
...
class NuscVoxelDataset(data.Dataset):
    def __init__(
        ...
        if self.data_cfgs.DATASET == 'nuscenes':
        ...
        if self.data_cfgs.DATASET == 'nuscenes_sweep':
            self.point_cloud_dataset = NuscenesSweepDataset(
                data_cfgs=data_cfgs,
                training=training,
                class_names=self.class_names,
                root_path=self.root_path,
