...
# x: SparseTensor, z: PointTensor
# return: PointTensor
def voxel_to_point2(x, z, nearest=False):
    if z.idx_query is None or z.weights is None or z.idx_query.get(
            x.s) is None or z.weights.get(x.s) is None:
        off = get_kernel_offsets(2, x.s, 1, device=z.F.device)
        old_hash = F.sphash(
        ...
        pc_hash = F.sphash(x.C.to(z.F.device))
        idx_query = F.sphashquery(old_hash.detach().cpu(), pc_hash.detach().cpu()).to(x.C.device)
        weights = F.calc_ti_weights(z.C, idx_query,
                                    scale=x.s[0]).transpose(0, 1).contiguous()
        idx_query = idx_query.transpose(0, 1).contiguous()
        if nearest:
...
# x: SparseTensor, z: PointTensor
# return: PointTensor
def voxel_to_point_fov(x, z, nearest=False):
    off = get_kernel_offsets(2, x.s, 1, device=z.F.device)
    old_hash = F.sphash(
        torch.cat([
            torch.floor(z.C[:, :3] / x.s[0]).int() * x.s[0],
