...
from .semantickitti_ms_mm import SemantickittiMsMmDataset
...
class SemkittiVoxelMsMmDataset(data.Dataset):
    def __init__(
        self,
        data_cfgs=None,
        training=True,
        ...
        if self.data_cfgs.DATASET == 'semantickitti_ms_mm':
            self.point_cloud_dataset = SemantickittiMsMmDataset(
                data_cfgs=data_cfgs,
                training=training,
        ...
        self.eval_range = self.data_cfgs.get('EVAL_RANGE', [0,10000])
    ...
    def get_single_sample(self, index, voting_idx=0):
        ...
        if self.eval_range[1] < 200:
            distance = np.linalg.norm(point[:, :2], axis=-1)
            distance_mask = (distance >= self.eval_range[0]) & (distance < self.eval_range[1])
            print(distance.min(), distance.max(), self.eval_range[0], '~', self.eval_range[1], '  ', distance_mask.shape[0], '=>', distance_mask.sum())
            point_label[~distance_mask] = 0
