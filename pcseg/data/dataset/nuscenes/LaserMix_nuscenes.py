...
def lasermix_aug_(xyzi_sup, label_sup, xyzi_unsup, label_unsup, strategy='mixture', return_strategy=False):
    ...
    inc_sup = np.arctan2(xyzil_sup[:, 2], rho_sup) / np.pi * 180  # inc = arctan(z / (x^2 + y^2)^(1/2)) / np.pi * 180
    inc_unsup = np.arctan2(xyzil_unsup[:, 2], rho_unsup) / np.pi * 180  # inc = arctan(z / (x^2 + y^2)^(1/2)) / np.pi * 180
    ...
    if strategy == 'inc3phi1':
        a = 0.0
        b =  -10.0
        xyzil_sup_p1 = xyzil_sup[inc_sup > a]
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= a, inc_sup > b)]
        xyzil_sup_p3 = xyzil_sup[inc_sup <= b]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > a]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= a, inc_unsup > b)]
        xyzil_unsup_p3 = xyzil_unsup[inc_unsup <= b]
        ...
        xyzil_sup_p1 = xyzil_sup[inc_sup > a]
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= a, inc_sup > b)]
        xyzil_sup_p3 = xyzil_sup[np.logical_and(inc_sup <= b, inc_sup > c)]
        xyzil_sup_p4 = xyzil_sup[inc_sup <= c]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > a]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= a, inc_unsup > b)]
        xyzil_unsup_p3 = xyzil_unsup[np.logical_and(inc_unsup <= b, inc_unsup > c)]
        xyzil_unsup_p4 = xyzil_unsup[inc_unsup <= c]
        ...
        xyzil_sup_p2 = xyzil_sup[np.logical_and(inc_sup <= a, inc_sup > b)]
        xyzil_sup_p3 = xyzil_sup[np.logical_and(inc_sup <= b, inc_sup > c)]
        xyzil_sup_p4 = xyzil_sup[np.logical_and(inc_sup <= c, inc_sup > d)]
        xyzil_sup_p5 = xyzil_sup[inc_sup <= d]

        xyzil_unsup_p1 = xyzil_unsup[inc_unsup > a]
        xyzil_unsup_p2 = xyzil_unsup[np.logical_and(inc_unsup <= a, inc_unsup > b)]
        xyzil_unsup_p3 = xyzil_unsup[np.logical_and(inc_unsup <= b, inc_unsup > c)]
        xyzil_unsup_p4 = xyzil_unsup[np.logical_and(inc_unsup <= c, inc_unsup > d)]
        xyzil_unsup_p5 = xyzil_unsup[inc_unsup <= d]
