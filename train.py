...
def parse_config():
    ...
    parser.add_argument('--max_ckp_save_num', type=int, default=3,
                        help='max number of saved checkpoint')
    parser.add_argument('--merge_all_iters_to_one_epoch', action='store_true', default=False,
                        help='')
    parser.add_argument('--fix_part_param', action='store_true', default=False,
                        help='')
    # == evaluation configs ==
    ...
    parser.add_argument('--votes_min', type=int, default=0)
    parser.add_argument('--votes_max', type=int, default=10)
    parser.add_argument('--ceph_path', type=str, default=None)
    ...
    os.environ["PATH"] = "/xxx/cuda-11.3/bin:" + os.environ["PATH"]
    os.environ["LD_LIBRARY_PATH"] = "/xxx/cuda-11.3/lib64:" + os.environ["LD_LIBRARY_PATH"]
    os.environ["CUDA_HOME"] = "/xxx/cuda-11.3:" + os.environ["CUDA_HOME"]
    os.environ["PATH"] = "/xxxx/cmake-3.14.0-Linux-x86_64/bin:" + os.environ["PATH"]
    os.environ["CPLUS_INCLUDE_PATH"] = "/YourHome/PCSeg/package/torchsparse_dir/sphash/include:"
    os.environ["LD_LIBRARY_PATH"] = "/YourHome/PCSeg/package/torchsparse_dir/sphash/lib/:" + os.environ["LD_LIBRARY_PATH"]
    return args, cfgs
...
class Trainer:
    ...
    def __init__(self, args, cfgs):
        ...
        # tta
        cfgs.DATA.TTA = self.args.tta
        cfgs.DATA.SEQ = args.seq
        cfgs.DATA.VOTES_MIN = args.votes_min
        cfgs.DATA.VOTES_MAX = args.votes_max
        self.votes =  args.votes_max - args.votes_min
        self.ceph_path = args.ceph_path
        if self.ceph_path is not None:
            from pcseg.data.dataset.ceph import PetrelBackend
            self.petrel_client = PetrelBackend()
        self.cfgs = cfgs
        self.strict = cfgs.DATA.get('STRICT', True)
        ...
        if cfgs.DATA.DATASET.startswith('nuscenes'):
            num_class = 17
        elif cfgs.DATA.DATASET.startswith('semantickitti_ms_ms') or cfgs.DATA.DATASET.startswith('semantickitti_triple'):
            num_class = 26
        elif cfgs.DATA.DATASET.startswith('semantickitti') or cfgs.DATA.DATASET.startswith('scribblekitti'):
            num_class = 20
        ...
        if args.fix_part_param:
            model.fix_part_param()
    ...
    def remove_checkpoint(self, removed_epoch):
        ckp_name = self.ckp_dir / ('checkpoint_epoch_%d.pth' % removed_epoch)
        if 's3://' in str(ckp_name):
            self.petrel_client.remove(str(ckp_name))
        else:
            os.remove(str(ckp_name))
    ...
    def resume(self, filename):
        ...
        if self.strict is False:
            self.model.load_params(checkpoint['model_state'], strict=False)
        else:
    ...
    def evaluate(self, dataloader, prefix):
        ...
        for i, batch_dict in enumerate(dataloader):
            ...
            if self.args.tta and self.cfgs.DATA.DATASET.startswith('semantickitti'):
                file_name = ret_dict['name'][0]
                point_predict = ret_dict['point_predict_logits'][0]
                for count in range(1, self.votes):
                    point_predict += ret_dict['point_predict_logits'][count]

                seq_id, frame_id = file_name.split('/')[-3], file_name.split('/')[-1]
                if self.ceph_path is not None:
                    out_path = self.ceph_path + '/sequences/' + seq_id + '/predictions/' + frame_id.replace('bin', 'label')
                    self.petrel_client.save_bin(out_path, point_predict)
                    print('saved to ', out_path, 'point_predict: ', point_predict.shape, point_predict.dtype)
                else:
                    out_path = self.args.save_path + '/sequences/' + seq_id + '/predictions/' + frame_id.replace('bin', 'label')
                    if not os.path.exists(os.path.dirname(out_path)):
                        os.makedirs(os.path.dirname(out_path))
                    if self.args.save_score:
                        if (self.args.seq == -3) or (self.args.seq == -1):
                            save_file = point_predict.astype(np.float32)
                            save_file.tofile(out_path)
                            print('saved to ', out_path, 'save_file: ', save_file.shape, save_file.dtype)
                        else:
                            k = 3
                            point_predict_topk = np.sort(point_predict, axis=1)[:,::-1][:,:k]
                            point_predict_top1 = np.max(point_predict, axis=1)
                            assert (point_predict_topk[:,0] != point_predict_top1).sum() == 0
                            
                            print('max score ', point_predict_top1.max())
                            save_file = point_predict_topk
                            save_file = save_file.astype(np.float32)
                            
                            save_file.tofile(out_path)
                            print('saved to ', out_path, 'save_file: ', save_file.shape, save_file.dtype)
                    else:
                        point_predict = np.argmax(point_predict, axis=1)
                        save_file = np.expand_dims(point_predict, axis=1)
                        save_file = save_file.astype(np.uint32)
                        save_file.tofile(out_path)
                        print('saved to ', out_path, 'save_file: ', save_file.shape, save_file.dtype)
            elif self.args.tta and self.cfgs.DATA.DATASET.startswith('nuscenes'):
                file_name = ret_dict['name'][0].split('/')[-1]
                point_predict = ret_dict['point_predict_logits'][0]
                for count in range(1, self.votes):
                    point_predict += ret_dict['point_predict_logits'][count]

                if self.ceph_path is not None:
                    out_path = self.ceph_path + '/' + file_name
                    self.petrel_client.save_bin(out_path, point_predict)
                    print('saved to ', out_path, 'point_predict: ', point_predict.shape, point_predict.dtype)
                else:
                    out_path = self.args.save_path + '/' + file_name
                    if not os.path.exists(os.path.dirname(out_path)):
                        os.makedirs(os.path.dirname(out_path))
                    point_predict = np.argmax(point_predict, axis=1)
                    save_file = np.expand_dims(point_predict, axis=1)
        ...
        if self.args.tta:
            ...
            submisson_json_file = {"meta": meta}
            submisson_json_path = self.args.save_path.replace('/lidarseg/', '/') + '/' + 'submission.json'
            if not os.path.exists(os.path.dirname(submisson_json_path)):
                os.makedirs(os.path.dirname(submisson_json_path))
            with open(submisson_json_path, 'w') as f:
                json.dump(submisson_json_file, f)
            return {}
    ...
    def train(self):
        ...
        with tqdm.trange(
            ...
            for cur_epoch in tbar:
                ...
                if trained_epoch > self.args.max_ckp_save_num and self.rank == 0:
                    self.remove_checkpoint(trained_epoch - self.args.max_ckp_save_num)
                ...
                if (cur_epoch+1) % self.eval_interval == 0 or cur_epoch == self.total_epoch-1:
                    ...
                    if cur_epoch == self.total_epoch-1:
                        print('This experiment has finished')
                    if self.if_dist_train:
