...
from .semantickitti_ms_kd import SemantickittiMsKdDataset
from torchsparse import SparseTensor
from torchsparse.utils.collate import sparse_collate_fn
from torchsparse.utils.quantize import sparse_quantize
from itertools import accumulate
from tools.utils.common.seg_utils import aug_points_ms_gt
import copy

class SemkittiVoxelMsKdDataset(data.Dataset):
    def __init__(
        self,
        data_cfgs=None,
        training=True,
        ...
        if self.data_cfgs.DATASET == 'semantickitti_ms_kd':
            self.point_cloud_dataset = SemantickittiMsKdDataset(
                data_cfgs=data_cfgs,
                training=training,
    ...
    def get_single_sample(self, index, voting_idx=0):
        ...
        point_ms = pc_data['xyzret_ms'][:, :self.in_feature_dim]
        point_ms_gt = pc_data['xyzret_ms_gt'][:, :self.in_feature_dim]

        num_points_current_frame = point.shape[0]
        num_points_current_frame_ms = point_ms.shape[0]
        num_points_current_frame_ms_gt = point_ms_gt.shape[0]

        ret = {}
        if self.training:
            point[:, 0:3], point_ms[:, 0:3], point_ms_gt[:, 0:3] = aug_points_ms_gt(
                xyz=point[:, :3],
                xyz_ms=point_ms[:, :3],
                xyz_ms_gt=point_ms_gt[:, :3],
        ...
        clamp_mask_gt = (point_ms_gt[:, 0] >= point[:, 0].min()) & (point_ms_gt[:, 1] >= point[:, 1].min()) & (point_ms_gt[:, 2] >= point[:, 2].min())
        point_ms_gt = point_ms_gt[clamp_mask_gt]

        pc_ = np.round(point[:, :3] / self.voxel_size).astype(np.int32)
        pc_ms_ = np.round(point_ms[:, :3] / self.voxel_size).astype(np.int32)
        pc_ms_gt_ = np.round(point_ms_gt[:, :3] / self.voxel_size).astype(np.int32)
        ...
        pc_ms_gt_ -= coord_shift
        feat_ms_gt_ = point_ms_gt
        _, inds_ms_gt, inverse_map_ms_gt = sparse_quantize(
            copy.deepcopy(pc_ms_gt_),
            return_index=True,
            return_inverse=True,
        
        )
        if self.training and len(inds_ms_gt) > self.num_points:  # NOTE: num_points must always bigger than self.num_points
            raise RuntimeError('droping point')
            inds_ms_gt = np.random.choice(inds_ms_gt, self.num_points, replace=False)

        pc_ms_gt = pc_ms_gt_[inds_ms_gt]
        feat_ms_gt = feat_ms_gt_[inds_ms_gt]
        lidar_ms_gt = SparseTensor(feat_ms_gt, pc_ms_gt)
        ...
        ret = {
            ...
            'lidar_ms_gt': lidar_ms_gt,
            'num_points_ms_gt': np.array([num_points_current_frame_ms_gt]), # for multi frames
        }
    ...
    def collate_batch(inputs):
        ...
        offset_ms_gt = [sample['lidar_ms_gt'].C.shape[0] for sample in inputs] # for point transformer
        ret.update(dict(
            offset_ms_gt=torch.tensor(list(accumulate(offset_ms_gt))).int()
        ))
