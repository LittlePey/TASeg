...
class SemantickittiDataset(data.Dataset):
    def __init__(
        ...
        self.eval_front = data_cfgs.get('EVAL_FRONT', False)
        self.eval_back = data_cfgs.get('EVAL_BACK', False)
    ...
    def __getitem__(self, index):
        ...
        if self.eval_front:
          front_mask = raw_data[:, 0] >=0
          raw_data = raw_data[front_mask]
          annotated_data = annotated_data[front_mask]
        if self.eval_back:
          back_mask = raw_data[:, 0] <0
          raw_data = raw_data[back_mask]
          annotated_data = annotated_data[back_mask]
        pc_data = {
            'xyzret': raw_data,
